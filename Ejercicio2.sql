...
#4. Obtener nombre y salario de todos los empleados#
...
SELECT e.nombre,(e.sal_emp+e.comision_emp+500)
FROM empleados e, departamentos d
...
SELECT sal_emp, comision_emp, (sal_emp+comision_emp) AS SumaTotal, nombre
FROM empleados
...
SELECT nombre_depto	
FROM departamentos
WHERE nombre_depto<> 'Ventas' OR 'Investigación' OR 'Mantenimiento';

#20. Mostrar el salario más alto de la empresa. 
SELECT MAX(sal_emp), nombre
FROM empleados;
...
SELECT d.nombre_depto, AVG(e.sal_emp)
FROM empleados e, departamentos d
...
SELECT COUNT(d.id_depto), d.nombre_depto, e.cod_jefe, e.nombre, e.cargo_emp 
FROM departamentos d, empleados e
WHERE d.id_depto=e.id_depto 
GROUP BY d.nombre_depto
HAVING COUNT(d.id_depto)>=2 AND e.cargo_emp LIKE 'JEFE%';
...
FROM empleados e
WHERE (
SELECT avg(e.sal_emp))
HAVING e.sal_emp>=avg(e.sal_emp);
